...
class GUI:
    def __init__(self):
        ...
        self.__frame = Frame(self.__main_window, bg='black', padx=10, pady=10)
        self.__frame.grid(column=0, row=0)
        self.__main_window.columnconfigure(0, weight=1)
        self.__main_window.rowconfigure(0, weight=1)

        TITLE = Label(self.__frame, text="COVID-19 Contact Tracing App", font= ('Bauhaus 93', 23), foreground='white', bg='black')
        ...
        # Ask for user's age
        age_label = Label(self.__frame, text="Age: ", font= ('OCR A Extended', 12), foreground='white', bg= 'black')
        ...
        vaccination_label = Label(self.__frame, text="Vaccination Status: ", font= ('OCR A Extended', 12), foreground='white', bg="black")
        ...
        # Make an add button 
        add_button = Button(self.__frame, text="Add Entry", font= ('Franklin Gothic Heavy', 12), foreground='White', bg='black', width= 20, command=self.__add_entry)
        add_button.grid(row=self.__frame.grid_size()[1], column=0, columnspan=2, pady=10)

        # Make a Search Button
        search_button = Button(self.__frame, text="Search Entry", font=('Franklin Gothic Heavy', 12), foreground='White', bg='black', width= 50, command=lambda:show_search_window(self.__main_window))
...
def user_entry(master, user_name):
    entry_label = Label(master, text=user_name, font= ('OCR A Extended', 12), foreground='white', bg='black')
    entry = Entry(master, width = 40,foreground='black', bg='white')
